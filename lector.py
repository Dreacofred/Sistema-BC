...
st.sidebar.divider()
st.sidebar.success("💎 NIVEL: GEMINI 2.5 PRO (GENERACIÓN NUEVA)")

# ==========================================
# 3. CONEXIÓN PRO (LIMPIA Y DIRECTA)
# ==========================================
# Conexión limpia usando la API Key
    ...
    if f_factura and st.button("🔍 ANALIZAR CON IA PRO", use_container_width=True):
        with st.spinner("Procesando con la máxima precisión..."):
            try:
                ...
                # ACÁ ESTÁ LA CLAVE: Usamos el modelo exacto que nos devolvió tu servidor
    ...
    if archivo and st.button("🚀 Extraer Datos con Pro"):
        with st.spinner("Analizando con motor Pro..."):
            try:
                ...
                res = cliente.models.generate_content(
                    model='gemini-2.5-pro', 
                    contents=["Extraé datos de esta factura en formato JSON puro", mat]
                )
